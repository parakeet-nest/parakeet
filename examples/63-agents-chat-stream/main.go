...
func main() {
	...
	for i := 0; i < 10; i++ {
		...
		// Bob's turn
		ui.Println(colors.Green, "🐼 Bob: ")

		bobResponse, _ = flockOrchestrator.RunStream(bob, conversation, map[string]interface{}{}, func(answer llm.Answer) error {
			ui.Print(colors.Green, answer.Message.Content)
			return nil
		})
		...
		// Sam's turn
		ui.Println(colors.Yellow, "🐻 Sam: ")

		samResponse, _ = flockOrchestrator.RunStream(sam, conversation, map[string]interface{}{}, func(answer llm.Answer) error {
			ui.Print(colors.Yellow, answer.Message.Content)
			return nil
		})
