...
func TestLevenshteinDistance(m *testing.T) {

	var docs = []string{
		`Michael Burnham Discovery`,
		`James Tiberius Star Trek`,
		`Jean-Luc Picard Next Generation`,
		`Lieutenant Philippe Charrière the Silent Sentinel`,
	}

	userContent := `Jean-Luc Picard?`

	// Calculate Levenshtein distance for every document
	// the lowest distance is related to the best similarity
	for idx, doc := range docs {
		levenshteinDistance := similarity.LevenshteinDistance(userContent, doc)
		fmt.Println( idx, ":", "dist", levenshteinDistance, "[", userContent, "]", "[", doc, "]")
	}

	expected := 16
	levenshteinDistance := similarity.LevenshteinDistance(userContent, docs[2])
	if levenshteinDistance > expected {
		m.Fatal("unexpected result")
	}
